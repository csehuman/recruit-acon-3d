...
test.describe('4. 월간 베스트 탭 확인', () => {
    ...
    test('4-1 기본 정보 확인 (전체 탭)', async () => {
        await expect(page.getByText('이번달 Top 30')).toHaveCount(1);
    ...
    test('4-4 목록 아이템 숫자 형식 확인', async () => {
    ...
    test('4-7 목록 아이템 제목 형식 확인', async () => {
    ...
    test('4-8 목록 아이템 파일 형식 확인', async () => {
    ...
    test('4-9 목록 아이템 가격 형식 확인', async () => {
        const priceListForItems = await page.locator('span.sc-9fb6eb33-13.cFlCeC');
        const priceListForItemsCount = await priceListForItems.count();
    ...
    test('4-10 목록 아이템 조회수 형식 확인', async () => {
