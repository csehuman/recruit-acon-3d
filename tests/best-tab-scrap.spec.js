...
test.describe.serial('베스트 탭 스크랩 가능 확인', () => {
    ...
    test.beforeAll(async ({ browser, contextOptions, browserName }) => {
        ...
        await page.getByPlaceholder('비밀번호').type(process.env.VALID_PASSWORD_BROWSERS, { delay : 100 }); 
    ...
    test.describe.serial('2-11 목록 아이템 스크랩 기능 확인', () => {
        test('2-11.2 일간 탭으로 이동하고 스크랩 누르기', async () => {
            ...
            for (let i = 0; i < scrapButtonForItemsCount; i++) {
                ...
                await scrapButtonForItems.nth(i).click({ 'delay' : 100 });
